...
if(isset($_POST['upd_contacts']))
  {
    $frm_data=filteration($_POST);
    $q="UPDATE contact_details SET `adress` = ?, `gmap` = ?,  `pn1` = ? , `pn2` = ?, `email` = ? ,`insta` = ? WHERE id=?";
    $values=[$frm_data['adress'], $frm_data['gmap'], $frm_data['pn1'],$frm_data['pn2'], $frm_data['email'], $frm_data['insta'],1];
